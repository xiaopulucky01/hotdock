 ...
 * Events: meta | reasoning | content | done | error
 */
  ...
  const flushEvent = () => {
    ...
    switch (event) {
      case 'meta':
        ...
        break
      case 'reasoning':
        handlers.onReasoning?.(String((payload as { delta?: string }).delta ?? ''))
        break
      case 'content':
        handlers.onContent?.(String((payload as { delta?: string }).delta ?? ''))
        break
      case 'done':
